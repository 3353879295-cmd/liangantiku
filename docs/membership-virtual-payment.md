...
前端继续共用进行中的购买请求。服务端先创建 PREPARED 订单，再生成支付双签名；金额、商品、数量和环境全部由服务端固定。登录 code 必须换取与可信 SDK OPENID 一致的会话。

虚拟支付每个商户单号只用于一次调起。收到微信明确取消回调后，客户端持久化取消意图并调用 `cancelPayment`；服务端先对已调起且尚未取消的订单尽力查单，已确认付款优先返回到账结果。其他情况按可信账户和订单原子记录 `purchase_cancelled_at`，释放本次购买占位；查单错误不冒充付款失败或平台关单。前端恢复购买按钮，下一次点击用新的 requestId 和订单号支付。取消报告因断网失败时保留意图，重新进入页面后重试报告，不自动发起付款。

已有 PAYMENT_STARTING / PAYMENT_UNKNOWN / PENDING 缓存不再禁用购买按钮。用户点击“重新支付”后，先查旧单：确认付款则直接展示权益；查询失败保留旧单供下次重试；未付款则取消原购买占位并继续创建新单。单纯刷新页面只恢复状态，不取消未知订单或新建订单。服务端返回 `purchaseCancelled` 和绑定 localOrderId 的 `cancelledOrderId`，让旧客户端缓存可在恢复时清理。

取消购买占位不等于微信关单，不伪造 CLOSED/FAILED，也不证明一定没有扣款。旧单仍参与后台查单、发货和退款处理；晚到的真实付款仍按原有平台交易唯一锁发放权益，同一平台交易不会重复延期。不同订单若都实际付款仍属于两笔真实购买，不能用本地取消标记撤销平台扣款。前端 success 不能直接开通会员。上线需同时部署 membership 云函数并更新小程序，真机验证取消、退出重进和再次支付。
