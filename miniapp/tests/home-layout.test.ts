...
const behavior = readFileSync(join(homeRoot, 'index.ts'), 'utf8');

const rule = (selector: string): string =>
  styles.match(new RegExp(`\\${selector}\\s*\\{([^}]*)\\}`, 's'))?.[1] ?? '';

describe('home action layout contract', () => {
  it('prioritizes random practice while clearing the fixed tab bar', () => {
    expect(markup).toContain('action-card--{{item.id}}');
    expect(behavior).toContain("emphasized: action.id === 'random'");
    expect(rule('.action-card--emphasized')).toMatch(/grid-column:\s*1\s*\/\s*-1;/);
    expect(rule('.action-card--chapter')).toMatch(/grid-column:\s*auto;/);
